...
export const updateVendor = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user?.role !== "OWNER" && req.user?.role !== "MANAGER") {
      return res
        .status(403)
        .json({ error: "Only OWNER and MANAGER can update vendors" });
    }
...
export const deleteVendor = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user?.role !== "OWNER" && req.user?.role !== "MANAGER") {
      return res
        .status(403)
        .json({ error: "Only OWNER and MANAGER can delete vendors" });
    }
    ...
    if (
      ...
      return res.status(400).json({
        error: "Cannot delete vendor with existing receipts or deliveries",
      });
    }
    res.status(500).json({ error: handlePrismaError(error) });
  }
